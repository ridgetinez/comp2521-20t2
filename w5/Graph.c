...
int reachableR(Graph g, Vertex start, Vertex *visited, Vertex *reachables, int nReachables);
/* Populates the reachables array to contain all nodes that are reachable from `start`.
   Returns the number of elements found, and populated, in the reachables array.
   Suppose that reachables always has enough space to keep adding elements on. */
int reachable(Graph g, Vertex start, Vertex *reachables)
{ 
   return reachableR(g, start, NULL, reachables, 0);
}
...
/* Return back whether an edge list is an Euler path on graph G */
bool isEulerPath(Graph g, Edge *edges, int nE)
{
   return 0;
}